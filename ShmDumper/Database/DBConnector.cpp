...
{
	std::stringstream szQuery;
	szQuery << "INSERT INTO minutely_data_1(epoch_time, trading_symbol, exchange_id, price_open, price_high, price_low, price_close, volume, open_interest) values( "
	<< md->m_ulExchangeTimeStamp << ",'"
	<< md->m_szScripCode << "',"
	<< md->m_eExchangeType << ","
	<< md->m_dPriceOpen << ","
	<< md->m_dPriceHigh << ","
	...
	if(mysql_query(&m_MySql, (char*)(szQuery.str().c_str())))
	{
		pthread_mutex_unlock(m_MySqlLock);
		std::cout << szQuery.str() << std::endl;
		std::cout << "DBError while inserting minutely data" << std::endl;
		return false;
	}
	...
	pthread_mutex_unlock(m_MySqlLock);
	//std::cout << "successfully inserted minutely data" << std::endl;
	return true;
}
